...
<%@page import="com.quix.aia.cn.imo.data.city.City"%>
<%@page import="java.util.ArrayList"%>
<%@page import="com.quix.aia.cn.imo.mapper.CityMaintenance"%>
	...
	var dialog = $('<p>'+str+'</p>').dialog({
        ...
        buttons: {
            "<%=localeObj.getTranslatedText("Yes")%>": function() {
            	window.location.href = 'FormManager?key=CITY_DIST_Add&type=DELETE&CITY_CODE='+code;
            },
...
<div id="maincontainer">
			<div class="head">
				<h2 align="center"><%=localeObj.getTranslatedText("City District Maintenance")%></h2>
			</div>
			<div class="content" style="background-color:#ffffff;">
				...
				<input type="hidden" name="token" id="token" value="<%=request.getSession().getAttribute("Token")+"" %>">
						...
						<tr>                         
                      				<td>	                                    				
                       				<label><%=localeObj.getTranslatedText("District Name")%></label>
                         	 			 ...
                         	 			 <td>
                          				<select name="district1" id="district1"  class="comboObj"  >
                          				 <option value="0" ><%=localeObj.getTranslatedText("Select")%></option>
                         						<%if(listDist!=null){ 
    											for(District dist:listDist){
    									%>
		          					<option value="<%=dist.getDistrictCode() %>" ><%=dist.getDistrictName() %> </option>
                   ...
                   <td>
                   <select class="comboObj" id="city1" name="city1">
                  	 <option value="0" ><%=localeObj.getTranslatedText("Select")%></option>
                  			<%if(listcity!=null){ 
							...
							<table class="subFormDesign">
								<thead>
										<tr>
											...
											<th><label><%=localeObj.getTranslatedText("City Name")%></label></th>
											 <th width="20%"><label><%=localeObj.getTranslatedText("Action")%></label></th> 
										</tr>
								</thead>
								<tbody>
									<%if(pager != null){ %>
