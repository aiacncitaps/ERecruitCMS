...
<ul class="table_list">
		<li class="cf title">
			...
			<p class="column3" style="width:20%;"><span>排序</span></p>
...
function squence(){
	...
	if(index != ''){
		...
		$.ajax({
			url : 'updateTenEleSqu',
			data : 'id='+arrSubId.join(',')+'&squence='+arrSqu.join(',')+'&type=children',
			dataType : "json",
		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i = 0;i<list.length;i++){
					...
					var contentTypes;
					var fileNames;
					if(bean.nextFlag == false){
						fileIds= bean.fileIds.split(',');
						fileTypes=bean.fileTypes.split(',');
						contentTypes= bean.contentTypes.split(',');
						fileNames=bean.fileNames.split(',');
					}
					var tr = '';
						tr += '		<li class="cf">';
						tr += '		<p class="column1" style="width:19.8%;"><span>{0}</span></p>'.format(bean.name);
						...
						var nextFlag=bean.nextFlag;
						if(nextFlag){
							tr += '			<a class="next" href="javascript:;" title=\"下一个\"onclick="childrenNext(\'{0}\');"></a>'.format(bean.id);
						}
						tr += '			<a class="edit" href="javascript:;" onclick="edit(\'{0}\',\'{1}\');"></a>'.format(bean.id,bean.resourceId);
	...
	function edit(id,resourceId){
		location.href = 'childrenPre?id=' + id +"&resourceId="+resourceId;
	}
	function childrenNext(id){
		//alert(id);
		location.href = 'childrenNextIndex?childrenId=' + id;
	}
