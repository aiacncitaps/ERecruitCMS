...
import com.quix.aia.cn.imo.data.addressbook.CandidateFamilyInfoId;
 ...
 * @version 1.0
 */
public class CandidateFamilyInfoMaintenance {
	static Logger log = Logger
			.getLogger(CandidateFamilyInfoMaintenance.class.getName());
	...
	public void saveOrUpdate(AddressBook addressBook, Session session) {
		...
		try {
			...
			for (CandidateFamilyInfo candidateFamilyInfo : addressBook.getCandidateFamilyInfos()) {
				...
				familyInfoId.setIosAddressCode(candidateFamilyInfo.getIosAddressCode());
	...
	/*	Session session = null;
		...
		try {
			...
			criteria.add(Restrictions.eq("familyInfoId.addressCode", addressBook.getAddressCode()));
			list = criteria.list();
			
			for(Iterator itr = list.iterator();itr.hasNext();){
				candidateFamilyInfo = (CandidateFamilyInfo) itr.next();
	 ...
	 * 
	 * @param list
	 * @return Set<CandidateFamilyInfo> List of Class Object
	 * 
	 */
	public Set<CandidateFamilyInfo> readCandidateFamilyInfos(
			Set<CandidateFamilyInfo> list) {

		for (CandidateFamilyInfo candidateFamilyInfo : list) {
			candidateFamilyInfo.setIosAddressCode(candidateFamilyInfo.getFamilyInfoId().getIosAddressCode());
			candidateFamilyInfo.setFamilyInfoId(null);
		}
		return list;
	}
